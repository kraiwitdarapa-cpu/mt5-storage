...
// --- Two-candle NET DIRECTION check (PATCH: majority/average direction) --
// Previously required both of the last 2 candles to be the SAME color
// (strict consecutive-color rule). That was too strict and caused missed
// trend continuation entries. Now: the two candles no longer need to
// match colors - we sum their signed bodies (close-open) and use the
// NET direction. Bull+Bull, or a mixed pair whose net body sum is still
// bullish (larger bull body outweighs the smaller bear body), both PASS.
// Only when the net sum is bearish (or exactly zero) does BUY fail.
// H4/H1: both candles must still also pass IsBigBody() (unchanged gate).
// M30/M15: direction only, same as before.
bool HasTwoBullCandles(const string symbol, ENUM_TIMEFRAMES tf)
{
   ...
   double netDirection = (r[0].close - r[0].open) + (r[1].close - r[1].open);
   if(netDirection <= 0.0) return false; // net bearish or flat -> not a bullish pair
...
bool HasTwoBearCandles(const string symbol, ENUM_TIMEFRAMES tf)
{
   ...
   double netDirection = (r[0].close - r[0].open) + (r[1].close - r[1].open);
   if(netDirection >= 0.0) return false; // net bullish or flat -> not a bearish pair
   ...
   if(isBullTrendFollowing || isCounterBuy)
   {
      ...
      if(currentPrice <= buyThreshold)
      {
            ...
            (MathMin(m15Rates[1].open, m15Rates[1].close) - m15Rates[1].low > MathAbs(m15Rates[1].close - m15Rates[1].open) * 2))
         {
            ...
            // Canonical flow: Step 3 (Lot) -> Step 4 (AI Confidence) -> Step 5
            // (TEQS, Trend-Following only) -> Step 6 (Execution). TEQS and AI
            // are both enforced inside Cr_ExecuteScoredOrder now, in that
            // order, immediately before order execution. checkTEQS=trendAligned
            // preserves the original rule that TEQS never runs for Counter-Trend.
            ...
            if(Cr_ExecuteScoredOrder(ORDER_TYPE_BUY, score, Cr_InpBaseSL_USD, Cr_InpTrendTP_USD,
                                trendAligned ? "Trend-Follow BUY" : "Counter-Trend BUY", !trendAligned, highImpactNews, trendAligned))
   ...
   else if(isBearTrendFollowing || isCounterSell)
   {
      ...
      if(currentPrice >= sellThreshold)
      {
            ...
            (m15Rates[1].high - MathMax(m15Rates[1].open, m15Rates[1].close) > MathAbs(m15Rates[1].close - m15Rates[1].open) * 2))
         {
            ...
            // Canonical flow: Step 3 (Lot) -> Step 4 (AI Confidence) -> Step 5
            // (TEQS, Trend-Following only) -> Step 6 (Execution), enforced
            // inside Cr_ExecuteScoredOrder. checkTEQS=trendAligned preserves
            // the original rule that TEQS never runs for Counter-Trend.
            ...
            if(Cr_ExecuteScoredOrder(ORDER_TYPE_SELL, score, Cr_InpBaseSL_USD, Cr_InpTrendTP_USD,
                                trendAligned ? "Trend-Follow SELL" : "Counter-Trend SELL", !trendAligned, highImpactNews, trendAligned))
               Cr_glbDashStatus = "ORDER OPENED";
         }
      }
   }
...
bool Cr_ExecuteSidewayFixedLotTrade(ENUM_ORDER_TYPE orderType, double lotSize, string reasonTag)
{
   ...
   // STEP 4: AI Confidence Score - pure Decision Gate (PASS/BLOCK only).
   // Previously enforced implicitly inside Cr_ExecuteOrderWithUSD_Risk (which
   // this function called directly); now that that helper is pure execution,
   // the same gate is made explicit here so Sideway keeps the exact same
   // universal AI coverage every Crypto order path has always had. TEQS
   // (Step 5) is intentionally NOT applied here - Sideway is excluded, per spec.
...
bool Cr_ExecuteScoredOrder(ENUM_ORDER_TYPE orderType, int score, double slUSD, double tpUSD, string reasonTag, bool isCounterTrend = false, bool highImpactNews = false, bool checkTEQS = false)
{
   double refPrice = SymbolInfoDouble(_Symbol, (orderType == ORDER_TYPE_BUY) ? SYMBOL_ASK : SYMBOL_BID);

   // --- STEP 3: Smart Risk Management (unchanged calculation) ---------------
   ENUM_RISK_LEVEL riskLvl = Cr_GetRiskLevel();

   // --- STEP 3: Priority 2 Entry Score + PATCH A5 Risk-Level Lot mapping ----
   ...
   // --- STEP 4: AI Confidence Score - pure Decision Gate (PASS/BLOCK only).
   // Same formula/threshold as before, only moved here (out of the low-level
   // execution helper) so every namespace enforces AI in the same place,
   // immediately after Lot Sizing (Step 3) and before TEQS (Step 5).
   double aiScore = Cr_AIConfidenceScore();
   if(aiScore < Cr_InpAIConfidenceThreshold)
      return false; // BLOCK - AI Confidence Score below threshold

   // --- STEP 5: TEQS Filter - Trend Following ONLY (checkTEQS=false for
   // Counter-Trend and Extreme, matching the original scope exactly).
   if(checkTEQS && !TEQS_CheckFilter(orderType))
   {
      Cr_glbDashStatus = glbTEQSLastReason + " -> BLOCKED";
      return false;
   }

   // --- STEP 6: Order Execution (Cr_ExecuteOrderWithUSD_Risk is now pure
   // execution - SL/TP calc + trade send only, no gating logic inside it).
...
bool Cr_ExecuteOrderWithUSD_Risk(ENUM_ORDER_TYPE orderType, double lotSize, double slUSD, double tpUSD, string comment)
{
   ...
   // NOTE: AI Confidence Score (Step 4) and TEQS (Step 5) are now both
   // enforced by the caller (Cr_ExecuteScoredOrder / Cr_ExecuteSidewayFixedLotTrade)
   // before this function is ever invoked. This function is pure Step 6
   // execution - SL/TP calc + trade send - identical formula/behavior to
   // before, just no longer doing gating internally.
   ...
   if(isBullTrendFollowing || isCounterBuy)
   {
      ...
      if(currentPrice <= buyThreshold)
      {
            ...
            (MathMin(m15Rates[1].open, m15Rates[1].close) - m15Rates[1].low > MathAbs(m15Rates[1].close - m15Rates[1].open) * 2))
         {
            ...
            // Canonical flow: Step 3 (Entry Score / Lot) -> Step 4 (AI
            // Confidence) -> Step 5 (TEQS, Trend-Following only) -> Step 6
            // (Execution). Entry Score itself does not depend on the AI/TEQS
            // outcome, so computing it first is a pure reorder.
...
void Gs_SidewayPriceEngine(bool highImpactNews, ENUM_RISK_LEVEL riskLevel)
{
   ...
   Gs_glbDashLot    = 0.01; // Step 3: Sideway lot is always fixed at 0.01 (see Gs_ExecuteSidewayPriceOrder)

   // Canonical flow: Step 3 (Lot, fixed 0.01) -> Step 4 (AI Confidence) ->
   // Step 6 (Execution). TEQS (Step 5) is intentionally NOT applied to the
   // Sideway Engine, per spec. AI Confidence is added here as the Final
   // Entry Filter for Gold/Silver Sideway too, mirroring the existing,
   // unmodified Gs_AIConfidenceScore() formula/weights already used by
   // Gs_RunTrendEngine and by Fx_RunSidewayPriceEngine's equivalent gate -
   // PASS or BLOCK only, never changes the Sideway zone, signal, or lot.
   MqlRates sidewayAiRates[];
   ArraySetAsSeries(sidewayAiRates, true);
   if(CopyRates(_Symbol, PERIOD_M15, 0, 3, sidewayAiRates) >= 3)
   {
      string aiBreakdown = "";
      double aiScore = Gs_AIConfidenceScore(orderType, sidewayAiRates, CurrentPrice, aiBreakdown);
      Gs_glbDashAIScore = aiScore;
      if(Gs_InpEnableAIConfidenceScore && aiScore < Gs_InpAIConfidenceThreshold)
      {
         Gs_glbDashStatus = StringFormat("AI BLOCKED (%.0f/100) %s", aiScore, aiBreakdown);
         return;
...
bool Gs_RunExtremeEngine(bool highImpactNews, ENUM_RISK_LEVEL riskLevel)
{
   ...
   if(currentBid >= extremeHighBound && !GlobalVariableCheck("ExtHighTrig_"+_Symbol))
   {
      ...
      // Canonical flow: Step 3 (Entry Score) -> Step 4 (AI Confidence) ->
      // Step 6 (Execution). TEQS (Step 5) is intentionally NOT applied to
      // the Extreme Engine, per spec. AI Confidence is added here as the
      // Final Entry Filter for Gold/Silver Extreme too, mirroring the
      // existing, unmodified Gs_AIConfidenceScore() formula/weights already
      // used by Gs_RunTrendEngine and by Fx_RunExtremeEngine's equivalent
      // gate - PASS or BLOCK only, never changes the Extreme signal/bounds.
      ...
      {
         string aiBreakdown = "";
         double aiScore = Gs_AIConfidenceScore(ORDER_TYPE_SELL, extAiRates, currentBid, aiBreakdown);
         Gs_glbDashAIScore = aiScore;
         if(Gs_InpEnableAIConfidenceScore && aiScore < Gs_InpAIConfidenceThreshold)
         {
            Gs_glbDashStatus = StringFormat("AI BLOCKED (%.0f/100) %s", aiScore, aiBreakdown);
   ...
   if(currentAsk <= extremeLowBound && !GlobalVariableCheck("ExtLowTrig_"+_Symbol))
   {
      ...
      // Canonical flow: Step 3 (Entry Score) -> Step 4 (AI Confidence) ->
      // Step 6 (Execution). Same missing-gate fix as the SELL branch above.
      int entryScore = Gs_CalculateEntryScore(true, 10, highImpactNews);
      ...
      {
         string aiBreakdown = "";
         double aiScore = Gs_AIConfidenceScore(ORDER_TYPE_BUY, extAiRates, currentAsk, aiBreakdown);
         Gs_glbDashAIScore = aiScore;
         if(Gs_InpEnableAIConfidenceScore && aiScore < Gs_InpAIConfidenceThreshold)
         {
            Gs_glbDashStatus = StringFormat("AI BLOCKED (%.0f/100) %s", aiScore, aiBreakdown);
...
bool Fx_RunExtremeEngine(bool highImpactNews, ENUM_RISK_LEVEL riskLevel)
{
   ...
   if(currentBid >= extremeHighBound && !GlobalVariableCheck("ExtHighTrig_"+_Symbol))
   {
      ...
      // Canonical flow: Step 3 (Entry Score) -> Step 4 (AI Confidence) ->
      // Step 6 (Execution). AI Confidence Score is the Final Entry Filter
      // for the Forex Extreme Engine too (Trend already had it). Reuses the
      // existing, unmodified Fx_AIConfidenceScore() formula/weights - PASS
      // or BLOCK only, never changes the Extreme signal/bounds above. ---
   ...
   if(currentAsk <= extremeLowBound && !GlobalVariableCheck("ExtLowTrig_"+_Symbol))
   {
      ...
      // Canonical flow: Step 3 (Entry Score) -> Step 4 (AI Confidence) ->
      // Step 6 (Execution), see note above.
      int entryScore = Fx_CalculateEntryScore(true, 10, highImpactNews);
